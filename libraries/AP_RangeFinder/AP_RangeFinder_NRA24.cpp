    ...
    AP_RangeFinder_Backend(_state, _params) 
{
    ...
    {
        uart = serial_manager.find_serial(AP_SerialManager::SerialProtocol_NRA24_DOWN, 0);
        if (uart != nullptr) {
            uart->begin(serial_manager.find_baudrate(AP_SerialManager::SerialProtocol_NRA24_DOWN, 0));
        }
    }
    else if (_params.orientation == 0)
    {
        uart = serial_manager.find_serial(AP_SerialManager::SerialProtocol_NRA24_FORWARD, 0);
        if (uart != nullptr) {
            uart->begin(serial_manager.find_baudrate(AP_SerialManager::SerialProtocol_NRA24_FORWARD, 0));
        }
    }
    else if (_params.orientation == 4)
    {
        uart = serial_manager.find_serial(AP_SerialManager::SerialProtocol_NRA24_BACK, 0);
        if (uart != nullptr) {
            uart->begin(serial_manager.find_baudrate(AP_SerialManager::SerialProtocol_NRA24_BACK, 0));
        }
    }
    else if (_params.orientation == 6)
    {
        uart = serial_manager.find_serial(AP_SerialManager::SerialProtocol_NRA24_LEFT, 0);
        if (uart != nullptr) {
            uart->begin(serial_manager.find_baudrate(AP_SerialManager::SerialProtocol_NRA24_LEFT, 0));
        }
    }
    else if (_params.orientation == 2)
    {
        uart = serial_manager.find_serial(AP_SerialManager::SerialProtocol_NRA24_RIGHT, 0);
        if (uart != nullptr) {
            uart->begin(serial_manager.find_baudrate(AP_SerialManager::SerialProtocol_NRA24_RIGHT, 0));
        }
    }
    else if (_params.orientation == 24)
    {
        uart = serial_manager.find_serial(AP_SerialManager::SerialProtocol_NRA24_UP, 0);
        if (uart != nullptr) {
            uart->begin(serial_manager.find_baudrate(AP_SerialManager::SerialProtocol_NRA24_UP, 0));
        }
    }
    ...
    AP_RangeFinder_Params& _params)
{
    if (_params.orientation == 25)
    {
        return AP::serialmanager().find_serial(AP_SerialManager::SerialProtocol_NRA24_DOWN, 0) != nullptr;
    }
    else if (_params.orientation == 0)
    {
        return AP::serialmanager().find_serial(AP_SerialManager::SerialProtocol_NRA24_FORWARD, 0) != nullptr;
    }
    else if (_params.orientation == 4)
    {
        return AP::serialmanager().find_serial(AP_SerialManager::SerialProtocol_NRA24_BACK, 0) != nullptr;
    }
    else if (_params.orientation == 6)
    {
        return AP::serialmanager().find_serial(AP_SerialManager::SerialProtocol_NRA24_LEFT, 0) != nullptr;
    }
    else if (_params.orientation == 2)
    {
        return AP::serialmanager().find_serial(AP_SerialManager::SerialProtocol_NRA24_RIGHT, 0) != nullptr;
    }
    else if (_params.orientation == 24)
    {
        return AP::serialmanager().find_serial(AP_SerialManager::SerialProtocol_NRA24_UP, 0) != nullptr;        
    }
    return false;
}
